...
export default {
    ...
    setup(props) {
        ...
        const handleDeleteSong = async (id) => {
            const remainSong = playlist.value.songs.filter(song => song.id != id)
            
            await updateDoc({ songs: remainSong })
        }

        return { playlist, ownership, handleDelete, handleDeleteSong, error }
    },
    components: {
        AddSong
    }
